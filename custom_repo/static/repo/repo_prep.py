...
zipfolder = os.path.join(os.getcwd(), 'zips')
...
class Generator:
    ...
    def _generate_md5_file( self ):
        try:
            ...
            # print("here")
            x = open( self.addons_xml ).read()
            # print(x)
            # print(type(x))
            # print(type(x.hexdigest()))

            m = md5( open( self.addons_xml ).read() ).hexdigest()
            # print("....ere.....")
            # save file
    ...
    def delete_zipaddons(self):
        listofaddons = os.listdir(repo_root)
        for addon in listofaddons:
            if '.zip' in addon:
                os.remove(os.path.abspath(addon))
                print("Deleted unwanted zips from addons folder")
...
class Compressor:
   ...
   def _recursive_zipper( self, dir, zip_file ):
            ...
            # get length of ch  aracters of what we will use as the root path
   ...
   def _create_compressed_addon_release(self):
       print("Start for Creating Zip File")
       print(self.addon_version_number)
       ...
       newdelzip = zipname
       finalpath = os.path.join(zipfolder, self.addon_name)
       ...
       os.rename( zippath, os.path.join(finalpath, zipname))
    #    removepath = os.path.join()
